...
def resize_to_content(image, width, height):
    ...
    if h < height:
        to_append = height - h
        image = cv2.copyMakeBorder(image, 0, to_append, 0, 0, cv2.BORDER_CONSTANT)
    if w > width:
        image = image[:, :width]
    if w < width:
        to_append = width - w
        image = cv2.copyMakeBorder(image, 0, 0, 0, to_append, cv2.BORDER_CONSTANT)
    return image
...
def get_offset(coordinates_1, coordinates_2):
    ...
    return tuple(a - b for a, b in zip(tuple(coordinates_1), tuple(coordinates_2)))
