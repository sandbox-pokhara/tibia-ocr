...
def convert(img: MatLike, font: Path | str = DEFAULT):
...
def convert_in_range(
    img: MatLike, upper: MatLike, lower: MatLike, font: Path | str = DEFAULT
):
    img = cv2.inRange(img, upper, lower)
    return convert_line(img, font=font)


def convert_red(img: MatLike, font: Path | str = DEFAULT):
...
def convert_number_threshed(img: MatLike, font: Path | str = DEFAULT):
    try:
...
def convert_number(img: MatLike, font: Path | str = DEFAULT):
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    _, img = cv2.threshold(img, 128, 255, cv2.THRESH_BINARY)
...
def convert_number_in_range(
    img: MatLike, lower: MatLike, upper: MatLike, font: Path | str = DEFAULT
):
    try:
...
def convert_number_red(img: MatLike, font: Path | str = DEFAULT):
    lowerb = np.array([(17, 17, 176)])
    upperb = np.array([255, 255, 255])
    return convert_number_in_range(img, lowerb, upperb, font=font)


def convert_number_green(img: MatLike, font: Path | str = DEFAULT):
